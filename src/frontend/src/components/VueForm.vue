<template>
    <form id="answer" @submit.prevent="submit" class="form-group">
                <div id="content-form">
                  <div class="row">
                    X:
                    <input class="form-control" type="text" name="x" v-model="x" @input="validateLength()"
                    ...
                    <div class="row">
                      Y:
                      <input class="form-control" type="text" name="y" v-model="y" @input="validateLength()"
                             onkeypress='return ((event.charCode >= 43 && event.charCode <= 57) || event.charCode === 101)'
                             placeholder="(-5..5)" autocomplete="off" onpaste="return false"><br>
                    </div>
                    <div class="row">
                      R:
                      <input class="form-control" type="text" name="R" v-model="r" @input="validateLength()"
                             onkeypress='return ((event.charCode >= 43 && event.charCode <= 57) || event.charCode === 101)'
                             placeholder="(0..5)" autocomplete="off" onpaste="return false"><br>
                    </div>
                    <div class="row pt-2">
                      <input :disabled="isDisabled" value="Подтвердить" @click="addHit" type="submit" class="btn btn-outline-primary">
                    </div>
                </div>
            </form>
</template>
